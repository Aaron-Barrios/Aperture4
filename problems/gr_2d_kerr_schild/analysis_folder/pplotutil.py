...
# Try to import 'datalib' from the aperture_plotting package
# If that doesn't exist, import the (likely stale) version from the current
# directory.
def import_datalib():
    from pathlib import Path
    home = Path.home()

    sys.path.append(os.path.join(home, "repos/aperture_plotting/"))
    try:
        import datalib
    except:
        import datalib_backup as datalib

    return datalib


def register_hotcold_cmap(matplotlib):
    import pickle
...
def output_plot_command(fname):
    import sys
    import os

    ofile = open(fname, 'w')
    line = "# From directory: " + os.getcwd() + "\n"
    ofile.write(line)
    line = " ".join(sys.argv) + "\n"
    ofile.write(line)
    ofile.close()
...
# Calculated the reconnected flux (in units of Az) from the Aperture
# 'data' object at dumpstep 'step'. If 'step' is None, use the current
# timestep of the 'data' object
def calc_reconnected_flux(data, step = None):
    if step is not None:
        data.load_fld(step)

    Az = calc_Az(data)[-1]
    iymid = Az.shape[0] // 2
    Az_midplane = Az[iymid,:]

    return np.max(Az_midplane) - np.min(Az_midplane)


# Calculate Az from the Aperture 'data' object
# Az will have the same shape as data.B1 and data.B2 (Bx and By)
def calc_Az(data, zero_policy = "lower"):
    allowed_zero_policies = ["lower", "extremum"]
    if zero_policy not in allowed_zero_policies:
        msg = "kwargs 'zero_policy' to calc_Az must be one of: %s" \
            % allowed_zero_policies
        msg += ". Got %s" % zero_policy
        raise ValueError(msg)
    ...
    # Do not include the pml in the calculation of Az
    ...
    # Reshape the input arrays to what is expected by the legacy function for
    # computing Az
    Az = calc_Az_core(xAz[0,:], yAz[:,0], BxAz.T, ByAz.T).T

    # Select the zero point of Az
    Az0 = 0.0
    if zero_policy == "lower":
        Az0 = Az[0,Nxr//2]
        Az = Az - Az0
    elif zero_policy == "extremum":
        Az0 = np.max(Az)
        Az = Az0 - Az 
...
# Return the blackbody spectrum
#    (15/pi^4) * x^3 / (exp(x) - 1) 
# Spectrum is normalized so that its integral over x equals 1.
BB_SMALL_ARG_THRESHOLD = 0.001
def blackbody(x):
    # x = eps / T
    # Magical logic that allows this function to work both on plain numbers
    # and arrays alike.
    ...
    idxsmall = np.where(x <  BB_SMALL_ARG_THRESHOLD)
    idxelse  = np.where(x >= BB_SMALL_ARG_THRESHOLD)
    out[idxsmall] = x[idxsmall]**2
    out[idxelse] = x[idxelse]**3 / (np.exp(x[idxelse]) - 1.)

    return out / (np.pi**4 / 15)


# Find gamma_kn = 1 / (4 eph) = 1 / (4 * 2.7 * kT)
# such that the effective gamma_rad equals gamma_max
ERROR_THRESHOLD = 0.01
def optimize_gkn_bb(gcool, gmax):
    gradt = np.sqrt(gcool * gmax)
    # The initial guess will be too high
    gkn0 = np.sqrt(gradt * gcool)
    searchmin = gcool
    searchmax = gradt
    error, gradic, gkn0, searchmin, searchmax = _update_gkn_guess(
        gcool, gmax, gkn0, searchmin, searchmax
    )
    ...
    return float("%.3g" % gkn0)


def _update_gkn_guess(gcool, gmax, gkn0, searchmin, searchmax):
    # Define grid on which to sample the seed-photon blackbody corresponding
    # to gkn0
    eph = 1. / (4 * gkn0)
    ktbb = eph / 2.7
    seedemin = eph / 100.
    seedemax = eph * 100.
    seede_edges = np.geomspace(seedemin, seedemax, num=100)
    seede_bins = np.sqrt(seede_edges[1:]*seede_edges[:-1])
    dseede = np.diff(seede_edges)

    # Define the grid of particle Lorentz factors
    bins_per_decade = 20
    ggridmin = gcool
    ggridmax = 10*gmax
    gvals = np.geomspace(
        ggridmin, ggridmax,
        num=int(np.ceil(np.log10(ggridmax / ggridmin)))*bins_per_decade
    )
    evals = gvals - 1.
    uvals = np.sqrt(gvals**2 - 1.)

    # Compute the IC power as a function of gamma
    gkn_bins = 1. / (4 * seede_bins)
    fkn_gvals = np.einsum("i,j->ij", gvals, np.ones(gkn_bins.shape))
    fkn_gkn   = np.einsum("i,j->ij", np.ones(gvals.shape), gkn_bins)
    fkn_vals  = fkn(fkn_gvals / fkn_gkn)
    pic = np.einsum("i,ij,j->i",
        uvals**2, fkn_vals, (dseede / ktbb) * blackbody(seede_bins / ktbb))
    pic = pic / gcool

    tcoolic = evals / pic
    tx = evals / gmax

    gradic = getZeroLocations(gvals, tcoolic - tx)
    if len(gradic) > 0:
        if len(gradic) > 1:
            raise ValueError("Got more than one solution for gradic.")
        gradic = gradic[0]
        if gradic > gmax:
            # gkn0 is too small
            gkn = np.sqrt(gkn0 * searchmax)
            searchmin = gkn0
        else:
            gkn = np.sqrt(gkn0 * searchmin)
            searchmax = gkn0
    else:
        # No solution implies the gradic is beyond ggridmax and, hence,
        # beyond gkn0
        gkn = np.sqrt(gkn0 * searchmax)
        searchmin = gkn0
        gradic = ggridmax
    error = np.abs(gradic - gmax) / gmax

    return error, gradic, gkn, searchmin, searchmax


# Given gamma_kn = 1 / (4 eph) = 1 / (4 * 2.7 * kT)
# and gamma_max (gmax), find gamma_cool (gcool)
# such that the effective gamma_rad equals gamma_max
ERROR_GCOOL_THRESHOLD = 0.01
def optimize_gcool_bb(gkn, gmax):
    # The initial guess will be too high
    gcool0 = gkn
    searchmin = gkn**2 / gmax
    searchmax = gkn
    error, gradic, gcool0, searchmin, searchmax = _update_gcool_guess(
        gkn, gmax, gcool0, searchmin, searchmax
    )
    while error > ERROR_GCOOL_THRESHOLD:
        error, gradic, gcool0, searchmin, searchmax = _update_gcool_guess(
            gkn, gmax, gcool0, searchmin, searchmax
        )

    return float("%.3g" % gcool0)


def _update_gcool_guess(gkn, gmax, gcool0, searchmin, searchmax):
    # Define grid on which to sample the seed-photon blackbody corresponding
    # to gkn0
    eph = 1. / (4 * gkn)
    ktbb = eph / 2.7
    ...
    evals = np.geomspace(
        egridmin, egridmax,
        num=int(np.ceil(np.log10(egridmax / egridmin)))*bins_per_decade
    )
    gvals = evals + 1
    uvals = np.sqrt(gvals**2 - 1.)
    ...
    if len(gradic) > 0:
        ...
        if gradic > gmax:
            # gcool0 is too big
            gcool = np.sqrt(gcool0 * searchmin)
            searchmax = gcool0
        else:
            gcool = np.sqrt(gcool0 * searchmax)
            searchmin = gcool0
    else:
        # No solution implies the gradic is beyond ggridmax and, hence,
        # gcool0 is (way) too big
        gcool = np.sqrt(gcool0 * searchmin)
        searchmax = gcool0
        gradic = egridmax + 1.
    error = np.abs(gradic - gmax) / gmax

    return error, gradic, gcool, searchmin, searchmax
